...
void mm_free(void *bp)
{
	...
	if(bp != NULL)
	{
		...
		if(next_block_alloc == FREEBLOCK)
		{
			current_block_size += GET_SIZE(HDRP(NEXT_BLKP(bp)));
			PUT(HDRP(bp), PACK(current_block_size, FREEBLOCK));
		}
		//TODO: Check if previous block is free or not by traversing through whole heap
		char *prev_block_ptr = heap_listp;
		while(NEXT_BLKP(prev_block_ptr) != bp)
		{
			prev_block_ptr = NEXT_BLKP(prev_block_ptr);
		}
